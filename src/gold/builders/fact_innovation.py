...
class FactInnovationBuilder(AnalyticalBaseBuilder):
    async def run(self, request: BuildRequest) -> DataFrame:
        ...
        # 🔹 Log transform - tutaj używamy tymczasowej ramki z fillna(0),
        # żeby VectorAssembler/MinMaxScaler nie padły. (tak jak przywróciłeś)
        ...
        # 🔹 Normalizacja (globalna, cross-country)
        ...
        # 🔹 Zamieniamy wektor na tablicę i JOINujemy z oryginalną df po (ISO, year)
        df_scaled_array = df_scaled.withColumn("scaled_array", vector_to_array(F.col("scaled_features")))
        # select kluczowe kolumny do joinu
        df_scaled_small = df_scaled_array.select("ISO3166-1-Alpha-3", "year", "scaled_array")
        # przywracamy znormalizowane wartości do głównej ramki przez join
        df = df.join(df_scaled_small, on=["ISO3166-1-Alpha-3", "year"], how="left")

        # 🔹 Obliczanie znormalizowanych kolumn: jeśli oryginalna wartość była NULL -> zostaw NULL, inaczej weź scaled_array[i]
        for i, c in enumerate(norm_cols):
            df = df.withColumn(
                f"{c}_norm",
                F.when(F.col(c).isNull(), F.lit(None)).otherwise(F.col("scaled_array").getItem(i))
            )

        # drop tymczasowej kolumny
        df = df.drop("scaled_array", "features", "scaled_features")
