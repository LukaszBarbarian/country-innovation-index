...
class RDExpenditureModelBuilder(SilverModelBuilder):
    """
    A model builder for the 'rd_expenditure' model in the Silver layer.
    
    This builder takes raw R&D expenditure data from the World Bank and enriches it by joining
    ...
    async def build(
        ...
        """
        Builds the 'rd_expenditure' model by joining R&D expenditure data with the country dimension table.
        ...
        Returns:
            DataFrame: An enriched Spark DataFrame for R&D expenditure by country and year.

        Raises:
            ValueError: If the 'COUNTRY' model or 'rd' dataset is missing.
        """
        ...
        # 2. Get the R&D expenditure data frame from datasets
        rd_df = datasets.get((DomainSource.WORLDBANK, "rd"))
        if not rd_df or rd_df.count() == 0:
            raise ValueError("No data found in the 'rd' data frame.")

        # 3. Normalize the R&D expenditure data columns
